...
const INITIAL_PLAYERS: GymPlayer[] = [
  { id: 'player-1', name: 'Juan Pérez', division: 'Platino', isg: 2450, approved: true, assignedRoutine: 'Full Body Pantafit' },
  { id: 'player-2', name: 'Camila Sosa', division: 'Oro', isg: 1980, approved: true },
  { id: 'player-3', name: 'Martín Quispe', division: 'Oro', isg: 2085, approved: true, assignedRoutine: 'Empuje + Tracción' },
  { id: 'player-4', name: 'Valentina Ríos', division: 'Plata', isg: 1620, approved: false },
];

const INITIAL_REQUESTS: JoinRequest[] = [
  { id: 'req-1', name: 'Nicolás Fernández', email: 'nico.fernandez@gmail.com', reason: 'Quiero entrenar en Pantafit' },
  { id: 'req-2', name: 'Agustina Ledesma', email: 'agus.ledesma@gmail.com', reason: 'Solicitud por recomendación de un amigo' },
];
...
export default function JugadoresGimnasioPage() {
  const [players, setPlayers] = useState<GymPlayer[]>(INITIAL_PLAYERS);
  const [requests, setRequests] = useState<JoinRequest[]>(INITIAL_REQUESTS);
  const [assignTarget, setAssignTarget] = useState<GymPlayer | null>(null);
  ...
  const assignLibrary = gymRoutines.length > 0 ? gymRoutines : ROUTINE_LIBRARY;
  ...
  const approveRequest = (id: string) => {
    ...
    setPlayers((prev) => [
      ...prev,
      { id: `player-${Date.now()}`, name: req.name, division: 'Bronce', isg: 1100, approved: true },
    ]);
  };
  ...
  const assignRoutine = (title: string) => {
    if (!assignTarget) return;
    setPlayers((prev) =>
      prev.map((p) => (p.id === assignTarget.id ? { ...p, assignedRoutine: title } : p))
    );
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-[#0D0D0D] px-4 py-2.5">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-white/40">
              {requests.length > 0 ? `+${requests.length} solicitudes` : 'vinculados'}
            </span>
          </div>
        </div>
      </header>
            ...
            {players.map((player) => (
              <div
                ...
                className={cn(
                  ...
                  player.approved ? 'border-white/5 bg-white/5' : 'border-yellow-500/40 bg-yellow-950/20'
                )}
              >
                <div className="flex min-w-0 flex-1 items-center gap-3">
                  <span
                    className={cn(
                      'flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl text-sm font-black',
                      player.approved ? 'bg-white/5 text-white/60' : 'bg-yellow-950/40 text-yellow-200'
                    )}
                  >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-xs font-bold text-[#FBBF24]">+{player.isg} ISG</span>
                      {!player.approved && (
                        <span className="rounded-full border border-yellow-500/40 bg-yellow-950/30 px-2.5 py-0.5 text-[9px] font-bold uppercase tracking-widest text-yellow-200">
                          En revisión
                        </span>
                      )}
                    </div>
                ...
                <div className="flex flex-shrink-0 items-center gap-2">
                  <div className="min-w-0 max-w-[10rem]">
                    ...
                    <p className={cn('truncate text-xs font-bold', player.assignedRoutine ? 'text-white' : 'text-white/30')}>
                      {player.assignedRoutine ?? 'Sin asignar'}
                    </p>
                  </div>
                  <button
          ...
          >
            ...
            <ul className="mt-4 space-y-2.5">
              {assignLibrary.map((routine) => {
                ...
                const isCurrent = assignTarget.assignedRoutine === title;
                const subtitle =
