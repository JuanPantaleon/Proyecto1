...
import { Flame, Trophy, Dumbbell, Activity, Award, ArrowRight, Calendar, Zap } from 'lucide-react';
...
const ROLE_SUBTITLE = 'Centro de control del atleta';

export default function JugadorDashboard() {
  const [activeRoutine, setActiveRoutine] = useState<ActiveRoutine | null>(null);
  const { profile } = useRole();
  const player: PlayerProfile = 'role' in profile && profile.role === 'player' ? profile : PLAYER_PROFILE;
  ...
  return (
    <div className="flex h-[100dvh] min-h-0 flex-col bg-black text-white overflow-hidden">
      ...
      <header className="flex-shrink-0 px-6 pb-4 pt-2">
        ...
        <p className="mt-0.5 text-sm font-medium text-white/40">{ROLE_SUBTITLE}</p>
        <div className="mt-4">
          <ProfileSwitcher />
        </div>
      </header>
