...
>(({ className, children, error, ...props }, ref) => {
  return (
    <SelectPrimitive.Trigger
      ref={ref}
      className={cn(
        'flex h-10 w-full items-center justify-between rounded-lg border border-input bg-input px-3 py-2 text-sm '
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-[var(--z-popover)] max-h-96 min-w-[8rem] overflow-hidden rounded-xl border border-popover-border bg-popover text-popover-foreground shadow-xl '
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    className={cn(
      ...
      + 'data-[highlighted]:bg-accent-subtle data-[highlighted]:text-accent-foreground '
